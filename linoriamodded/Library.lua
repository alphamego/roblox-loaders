...
ScreenGui.Name = "LinoriaModded_" .. string.sub(tostring({}), 8)
...
do
	...
	local b64decode = (crypt and crypt.base64 and crypt.base64.decode)
		...
		or base64decode
		or base64_decode
	local FontFolder = "LinoriaModded"
	local TtfPath = FontFolder .. "/ProggyClean.ttf"
	local JsonPath = FontFolder .. "/Plex.json"
	local FontUrl = (typeof(getgenv) == "function" and getgenv().LINORIAMODDED_FONT_URL)
		or "https://raw.githubusercontent.com/alphamego/roblox-loaders/main/linoriamodded/assets/ProggyClean.ttf"

	local function ensureFontFiles()
		if not (writefile and isfile and getcustomasset) then
			return false
		end
		if makefolder and not isfolder(FontFolder) then
			pcall(makefolder, FontFolder)
		end
		if not isfile(TtfPath) then
			local ok, body = pcall(function()
				return game:HttpGet(FontUrl, true)
			end)
			if ok and type(body) == "string" and #body > 1000 then
				writefile(TtfPath, body)
			elseif b64decode then
				return false
			else
				return false
			end
		...
		writefile(JsonPath, HttpService:JSONEncode(plex))
		return isfile(JsonPath)
	end

	local okFont = pcall(function()
		if ensureFontFiles() then
			CustomFontFace = Font.new(getcustomasset(JsonPath), Enum.FontWeight.Regular)
		end
	end)
	if not okFont then
		CustomFontFace = nil
...
local Library = {
    ...
    GhostDrag = false; -- when true, main window shows white outline while dragging
...
function Library:Create(Class, Properties)
    ...
    for Property, Value in next, Properties do
        ...
        local success, err = pcall(function()
            if Property == "Font" and Library.FontFace then
...
local function CreateGhostOutline(Target)
    local Ghost = Instance.new("Frame")
    Ghost.Name = "GhostDragOutline"
    Ghost.BackgroundTransparency = 1
    Ghost.BorderSizePixel = 0
    Ghost.Size = UDim2.fromOffset(Target.AbsoluteSize.X, Target.AbsoluteSize.Y)
    Ghost.Position = Target.Position
    Ghost.AnchorPoint = Target.AnchorPoint
    Ghost.ZIndex = 10000
    ...
    if Library.IsMobile == false then
        Instance.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                local UseGhost = Library.GhostDrag == true and IsMainWindow == true
                local Ghost = UseGhost and CreateGhostOutline(Instance) or nil
        ...
        InputService.TouchStarted:Connect(function(Input)
            ...
            if not Dragging and Library:MouseIsOverFrame(Instance, Input) and (IsMainWindow == true and (Library.CanDrag == true and Library.Window.Holder.Visible == true) or true) then
                ...
                Dragging = true
                if Library.GhostDrag == true and IsMainWindow == true then
                    Ghost = CreateGhostOutline(Instance)
                end
            end
        end)
...
function Library:GetTextBounds(Text, Font, Size, Resolution)
    ...
    local MeasureFont = (typeof(Font) == "EnumItem" and Font) or Enum.Font.Code
    local Bounds = TextService:GetTextSize(Clean, Size, MeasureFont, Resolution)
    return Bounds.X, Bounds.Y
end
...
function Library:CreateWindow(...)
    ...
    local WindowLabel = Library:CreateLabel({
        ...
        TextXAlignment = Enum.TextXAlignment.Left;
        TextColor3 = Library.FontColor2 or Library.FontColor;
        ZIndex = 1;
        Parent = Inner;
    })
    Library:AddToRegistry(WindowLabel, { TextColor3 = "FontColor2" })
...
do
        ...
        function Tab:ShowTab()
            ...
            TabButtonLabel.TextColor3 = Library.SelectedTabColor or Library.AccentColor
            if Library.RegistryMap[TabButtonLabel] then
                Library.RegistryMap[TabButtonLabel].Properties.TextColor3 = "SelectedTabColor"
            end
